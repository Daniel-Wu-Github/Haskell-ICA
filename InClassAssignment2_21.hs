...
eval env (Add e1 e2)   =
    ...
    in NumVal (n1 + n2)   --wrong sign, changed to addition
eval env (Mul e1 e2)   =
    let NumVal n1 = eval env e1
        NumVal n2 = eval env e2
    in NumVal (n1 * n2)  --wrong sign, changed to multiplication
eval env (Var x)       =
...
transform f expr = case expr of
    ...
    Mul e1 e2  -> f (Mul (transform f e1) (transform f e2))   -- did not transform multiplication
